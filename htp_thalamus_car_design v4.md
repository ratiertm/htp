...
**Rev 1.3**: Knowledge Loop MVP 삽입 — Stage 0 직후에 최소 지식 입출력 루프를 닫아 설계의 실효성을 조기 검증. "매일 쓰는 도구"가 되어야 아키텍처가 가치를 가진다는 원칙 반영
...
| 클래스 | 위치 | 신규/변경 | 역할 |
|--------|------|---------|------|
| `KnowledgeLoop` | `htp/knowledge/loop.py` | **신규** | 최소 지식 입출력 루프 (Stage 0.5 MVP) |
| `TextEncoder` | `htp/knowledge/encoder.py` | **신규** | 텍스트→64-dim 벡터 변환 프로토콜. 구현 교체 가능 |
...
| **0.5** | **Knowledge Loop** | loop_ingest_basic | 텍스트 입력 → vec 생성 → knowledge_log 저장 |
| | | loop_query_neighbor | 유사 텍스트 질의 → 관련 지식 반환 (cosine > 0.5) |
| | | loop_discover_cross_domain | 다른 source 지식 쌍에서 high-similarity 발견 |
| | | loop_text_encoder_interface | TextEncoder 프로토콜 준수 (교체 가능성 검증) |
| | | loop_empty_state | 지식 0건에서 query/discover가 에러 없이 빈 결과 반환 |
...
2. **루프를 먼저 닫는다** — 아무리 정교한 내부 메커니즘이 있어도, 실제 지식을 넣고 꺼내보지 않으면 설계의 실효성을 검증할 수 없다. 최소 입출력 루프를 조기에 확보하여 이후 Stage의 방향을 실사용 피드백으로 교정한다
3. **본선/실험 분리** — EmbeddingBridge는 외부 의존성(sLLM)이 크므로 별도 브랜치에서 검증
4. **각 Stage에 Go/No-Go** — 테스트 개수뿐 아니라 제품적 성공 기준을 명시
...
| **0.5** | Knowledge Loop MVP | 60+5 = 65 | 텍스트 입력→벡터 저장→관계 발견→텍스트 출력 루프 1회 완주. "본인이 실제로 써보고 싶다"는 느낌이 들면 Go | 입력 파이프라인 또는 출력 포맷 재설계 |
| **1** | Vector routing | 65+10 = 75 | tag mode 대비 routing miss 감소, empty route 0건 | RegionSignature 초기화/warmup 로직 재조정 |
| **2** | Hybrid 검증 | 75+3 = 78 | α=0.1→0.9 변화에 결과 급변(∆cosine>0.5) 없음 | α 스케줄링 또는 blending 함수 교체 |
| **3** | CoherenceGate | 78+6 = 84 | 의도적 conflict case에서 90% 이상 감지 | conflict_threshold 조정 또는 감지 알고리즘 변경 |
| **4** | ExternalRegion | 84+3 = 87 | LLMRegion이 NativeRegion 없이 RegionSignal 프로토콜 100% 준수 | 프로토콜 인터페이스 재정의 |
| **5** | Pipeline | 87+2 = 89 | throughput ≥ 1.5× 순차 대비, 결과 순서 보존 100% | 파이프라인 버퍼/동기화 로직 재검토 |
| **6** | EmbeddingBridge *(실험)* | 별도 | roundtrip cosine > 0.8, dim-tag 대비 품질 우위 | 모델 교체 또는 프로젝션 차원 조정 |
| **7** | Vector default | 89 재실행 | 실제 샘플 입력에서 tag mode보다 품질 우위 확인 | hybrid 모드 유지, 전환 연기 |
...
### Stage 0.5: Knowledge Loop MVP — 최소 지식 입출력 루프

#### 동기

기존 RAG가 "지식의 저장소"로 실패하는 근본 이유는, 지식 조각(Chunk)을 저장할 뿐 지식과 지식 사이의 구조적 관계를 발견하지 못하기 때문이다. HTP는 이 문제를 풀기 위해 설계되었지만, 현재 코드 4,406줄은 "인공 두뇌의 신경 회로 기판"이지 아직 "지식을 넣고 통찰을 꺼내는 도구"가 아니다.

아무리 정교한 라우팅과 CoherenceGate가 있어도, 실제 지식을 넣고 꺼내보지 않으면 설계의 실효성을 검증할 수 없다. Stage 7까지 진행한 후 "실제로 쓸 때는 다르게 동작해야 했다"를 발견하는 것보다, **지금 최소한의 루프를 닫고 매일 쓰면서 나오는 피드백으로 이후 Stage를 교정하는 것이 합리적**이다.

#### 목표

```
[텍스트 입력] → 벡터 변환 → Region 저장 → 관계 발견 → 텍스트 출력
```

이 루프가 한 번이라도 돌아가면, HTP는 "연구 프로젝트"에서 "매일 쓰는 도구"로 전환된다.

#### 구현 범위

```python
# knowledge_loop.py — Stage 0.5 MVP. 최소 지식 입출력 루프.
# 기존 Phase 1-5 구현 위에 얇은 CLI 레이어를 올린다.

class KnowledgeLoop:
    """
    최소 지식 관리 루프.
    
    입력: 텍스트 (웹에서 복사한 문단, 읽은 책의 요약, 떠오른 아이디어)
    저장: 64-dim 벡터로 변환 → RegionSignature에 누적
    발견: 신규 입력과 기존 지식의 유사도/충돌 감지
    출력: 관련 기존 지식 + 관계 해석 (터미널 텍스트)
    """
    
    def __init__(self, brain: BrainRuntime):
        self.brain = brain
        self.knowledge_log: list[KnowledgeEntry] = []  # 입력 이력
    
    def ingest(self, text: str, source: str = "") -> IngestResult:
        """
        지식 입력. 텍스트 → 64-dim 벡터 → 시스템에 주입.
        
        1단계: 텍스트 → 벡터 변환 (MVP에서는 간이 해싱)
        2단계: BrainRuntime.run()으로 라우팅 + 처리
        3단계: 기존 지식과의 유사도/충돌 보고
        """
        # 1) 텍스트 → 벡터 (MVP: TF-IDF + JL projection. 
        #    Stage 6 EmbeddingBridge로 교체 가능)
        vec = self._text_to_vec(text)
        
        # 2) BrainRuntime에 주입
        result = self.brain.run(vec)
        
        # 3) 기존 지식과 비교
        neighbors = self._find_neighbors(vec, top_k=5)
        conflicts = [n for n in neighbors if n.similarity < 0.3]
        resonances = [n for n in neighbors if n.similarity > 0.7]
        ...
        self.knowledge_log.append(entry)
    ...
    def query(self, question: str) -> QueryResult:
        """
        지식 질의. "이 주제와 관련된 내 지식은?"
        
        question → 벡터 → 가장 가까운 기존 지식 N개 반환
        + 지식 간 관계 요약 (MVP: 단순 유사도 기반)
        """
        q_vec = self._text_to_vec(question)
        neighbors = self._find_neighbors(q_vec, top_k=10)
        
        # 이웃 지식들 간의 상호 관계 (coherence matrix의 축소판)
        clusters = self._cluster_neighbors(neighbors)
        
        return QueryResult(
            question=question,
            relevant_knowledge=neighbors,
            clusters=clusters
        )
    
    def discover(self) -> list[Discovery]:
        """
        비지도 발견. "내 지식에서 예상치 못한 연결은?"
        
        모든 저장된 지식 쌍의 유사도를 스캔하여,
        다른 출처(source)인데 높은 유사도를 가진 쌍을 반환.
        = 서로 다른 도메인의 지식이 벡터 공간에서 가까운 경우
        = 창의적 연결의 후보
        """
        discoveries = []
        for i, a in enumerate(self.knowledge_log):
            for b in self.knowledge_log[i+1:]:
                if a.source == b.source:
                    continue  # 같은 출처는 건너뜀
                sim = cosine_similarity(a.vec, b.vec)
                if sim > 0.6:  # 다른 도메인인데 벡터가 가까움
                    discoveries.append(Discovery(
                        knowledge_a=a, knowledge_b=b,
                        similarity=sim,
                        insight=f"'{a.source}'의 지식과 '{b.source}'의 지식이 "
                                f"64-dim 공간에서 {sim:.2f} 유사도로 연결됨"
                    ))
    ...
    def _text_to_vec(self, text: str) -> np.ndarray:
        ...
        의도적으로 단순하게 구현한다:
        - TF-IDF (또는 BoW) → 고차원 희소 벡터
        - JL Lemma random projection → 64-dim
        
        품질은 낮지만, 루프를 닫는 것이 목적이다.
        Stage 6 EmbeddingBridge가 완성되면 이 함수만 교체하면 된다.
        인터페이스(str → np.ndarray[64])는 동일하게 유지.
        """
        # sklearn TfidfVectorizer + random projection
        # 또는 더 간단하게: hashlib 기반 feature hashing
        pass  # 구현 상세는 코딩 시 결정
    
    def _find_neighbors(self, vec, top_k):
        """Memory L3 PatternStore 또는 knowledge_log에서 nearest neighbor"""
        pass
...
class KnowledgeEntry:
    text: str
    vec: np.ndarray
    source: str                    # "뇌과학 논문", "분산시스템 책", "아이디어"
    timestamp: datetime
    neighbors: list                # 입력 시점의 이웃
    conflict_count: int

@dataclass
class Discovery:
    knowledge_a: KnowledgeEntry
    knowledge_b: KnowledgeEntry
    similarity: float
    insight: str                   # 인간 언어 해석 (MVP: 템플릿, Stage 4 이후: LLMRegion)
```
...
$ python -m htp.knowledge ingest --source "뇌과학" \
    "뇌의 기억은 주소가 아니라 내용으로 저장·인출된다"

# 결과:
# ✓ 저장 완료 (vec norm: 0.87)
# ◆ 유사 지식 3건:
#   [0.82] "content-addressable memory는 검색키가 내용 자체" (소프트웨어)
#   [0.71] "Hopfield network의 에너지 최소화가 패턴 인출" (AI)
#   [0.34] "Redis는 key-value 주소 기반 접근" (인프라)  ← 낮은 유사도 = 대조점
# ⚡ 발견: '뇌과학' 지식이 '소프트웨어' 지식과 0.82 유사도

# 지식 질의
$ python -m htp.knowledge query "병렬 처리"

# 비지도 발견
$ python -m htp.knowledge discover
# ⚡ Top 발견:
#   1. [0.78] 뇌과학:"시냅스 가소성" ↔ 소프트웨어:"온라인 학습" 
#   2. [0.71] AI:"attention" ↔ 뇌과학:"시상 게이팅"
#   3. [0.65] 분산시스템:"consensus" ↔ 뇌과학:"감마 동기화"
```
...
| 배제 항목 | 이유 | 해소 시점 |
|----------|------|----------|
| 고품질 임베딩 | EmbeddingBridge는 실험 트랙. MVP는 TF-IDF+JL로 충분 | Stage 6 |
| LLM 기반 통찰 생성 | LLMRegion 리팩토링 전. MVP는 템플릿 기반 텍스트 | Stage 4 |
| 웹 스크래핑/PDF 파싱 | 입력 파이프라인은 별도 유틸리티. MVP는 텍스트 직접 입력 | 별도 도구 |
| 시각화 UI | CLI로 충분. 시각화는 지식이 쌓인 후 가치가 생김 | Stage 5 이후 |
| 자동 공고화 (수면 시뮬레이션) | 기존 Memory L2→L3 경로가 이미 구현됨. MVP에서 수동 트리거 | 자동화는 후순위 |

#### 핵심 설계 결정: `_text_to_vec` 인터페이스 격리

```python
# 인터페이스 계약: str → np.ndarray shape (64,)
# 구현은 교체 가능:
#   MVP:     TF-IDF + JL random projection
#   Stage 6: EmbeddingBridge (sLLM 프로젝션)
#   장기:    ModalEncoder (continuous token)

class TextEncoder(Protocol):
    def encode(self, text: str) -> np.ndarray: ...
```

이 인터페이스가 `KnowledgeLoop`, `LLMRegion`, `RegionSignature` 모두에서 공유된다. 벡터 변환 품질이 올라갈 때마다 전체 시스템이 동시에 개선되는 구조.

#### 테스트

| 테스트 | 검증 대상 |
|--------|----------|
| loop_ingest_basic | 텍스트 입력 → vec 생성 → knowledge_log 저장 확인 |
| loop_query_neighbor | 유사 텍스트 질의 → 관련 지식 반환 (cosine > 0.5) |
| loop_discover_cross_domain | 다른 source의 지식 쌍에서 high-similarity 발견 |
| loop_text_encoder_interface | TextEncoder 프로토콜 준수 (교체 가능성 검증) |
| loop_empty_state | 지식 0건 상태에서 query/discover가 에러 없이 빈 결과 반환 |

#### Go/No-Go 기준

**Go**: 아래 시나리오가 실제로 동작하면 Go.

```
1. "뇌의 기억은 내용 기반으로 인출된다" (source: 뇌과학) 입력
2. "Hopfield network는 에너지 최소화로 패턴을 인출한다" (source: AI) 입력  
3. "Redis는 key로 value를 조회한다" (source: 인프라) 입력
4. discover 실행 → 뇌과학-AI 쌍이 뇌과학-인프라 쌍보다 높은 유사도로 발견됨
```

즉, **"다른 도메인인데 본질적으로 같은 이야기를 하는 지식"을 벡터 공간이 자동으로 발견하는지** 여부. TF-IDF+JL이라는 조잡한 임베딩으로도 이 정도가 되면, EmbeddingBridge로 교체했을 때 품질이 극적으로 올라갈 것임을 확신할 수 있다. 되지 않으면 벡터 변환 전략 자체를 재검토 — 이 경우 Stage 1-7의 전제가 흔들리므로, 조기에 발견하는 것이 중요하다.

**No-Go**: 위 시나리오에서 뇌과학-AI 유사도가 뇌과학-인프라 유사도보다 낮거나 구분이 안 되면 No-Go. TF-IDF의 어휘 중복 한계일 수 있으므로, feature hashing 차원 조정 또는 간이 word2vec 평균 벡터로 전환 후 재시도.

#### 이 Stage가 이후 모든 Stage에 미치는 영향

Knowledge Loop MVP는 단순한 추가 기능이 아니라, **이후 Stage의 품질 검증 도구**이다.

| 이후 Stage | MVP가 제공하는 검증 |
|-----------|-------------------|
| Stage 1 (Vector routing) | MVP의 실제 지식 입력을 vector routing 테스트 데이터로 사용 |
| Stage 2 (Hybrid) | MVP에서 축적된 지식으로 tag vs vector 라우팅 품질 실비교 |
| Stage 3 (CoherenceGate) | MVP의 cross-domain 발견이 CoherenceGate conflict와 어떻게 대응하는지 검증 |
| Stage 4 (LLMRegion) | MVP의 discover() 결과를 LLMRegion이 자연어로 해석하도록 연결 |
| Stage 6 (EmbeddingBridge) | MVP의 TF-IDF 결과 vs EmbeddingBridge 결과를 동일 데이터로 A/B 비교 |

### Stage 1: RegionSignature + CoreCells 벡터 모드

- **Plan**: §1 구현. `RegionSignature` 클래스 + CoreCells에 `_gate_vector()` 추가. `RoutingConfig`에서 파라미터 로드
- **Do**: `RoutingConfig.mode = "tag"` 기본값 유지. vector 모드는 테스트에서만 활성화. **Stage 0.5에서 축적된 실제 지식 데이터를 vector routing 테스트 입력으로 활용**
- **Check**: Signature 4개 + CAR 6개 = 75 테스트
...
- **Check**: Coherence 6개 테스트 (LSH fallback 포함) = 84 테스트
...
- **Check**: Pipeline 2개 테스트 (throughput + order preservation) = 89 테스트
...
- **Check**: 전체 89개 테스트를 vector 모드로 재실행
...
| MVP TF-IDF 임베딩 품질 | TF-IDF+JL은 의미적 유사도가 아닌 어휘 중복만 포착 | MVP의 목적은 루프를 닫는 것이지 품질이 아님. **"다른 도메인인데 비슷한 이야기"를 발견 못 하면** word2vec 평균 벡터로 전환. 궁극적으로 Stage 6 EmbeddingBridge가 해소 |
